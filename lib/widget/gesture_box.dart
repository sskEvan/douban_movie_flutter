...
class GestureBox extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _GestureBoxState();
  }
}

class _GestureBoxState extends State<GestureBox> with TickerProviderStateMixin {
  // 缩放动画控制器
  ...
  _onScaleUpdate(ScaleUpdateDetails details) {
    setState(() {
      if (details.scale != 1.0) {
        debugPrint('_onScaleUpdate: _scaling:' + details.toString());
        _scaling(details);
      } else {
        debugPrint('_onScaleUpdate: _dragging:' + details.toString());
        _dragging(details);
      }
    });
  }
  ...
  _animationOffset(Offset targetOffset) {
    debugPrint('targetOffset(${targetOffset.dx},${targetOffset.dy})');
    _offsetAnimController?.dispose();
