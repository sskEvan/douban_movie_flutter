...
class DragContainer extends StatefulWidget {
  ...
  DragContainer(
      ...
      : assert(drawer != null),
        ...
        assert(height != null),
        super(key: key){
    debugPrint('------------------------------------------------DragContainer:key=${key.toString()}');
  }
...
class DragContainerState extends State<DragContainer>
  ...
  AnimationController dragToggleController;
  ...
  void initState() {
    ...
    dragToggleController = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 500));
    maxOffsetDistance = widget.height * 0.5;
...
//    }
  ...
  @override
  void dispose() {
    offsetAnimalController.dispose();
    super.dispose();
  }
  ...
  void _handleDragEnd(DragEndDetails details) {
    ...
    offsetAnimalController.value = 0.0;
    ...
    ///easeOut 先快后慢
    final CurvedAnimation curve = CurvedAnimation(parent: offsetAnimalController, curve: Curves.easeOut);
    debugPrint('--------start:${start},end:${end}');
    offsetAnimation = Tween(begin: start, end: end).animate(curve)
      ..addListener(() {
        if (!onResetControllerValue) {
          offsetDistance = offsetAnimation.value;
          setState(() {});
        }
      });
    ///自己滚动
  ...
  void openDrawer() {
    var start = offsetDistance;
    var end = 0.0;
    debugPrint('openDrawer start:${start},end:${end}');
    dragToggleController.reset();
    final CurvedAnimation curve = CurvedAnimation(parent: dragToggleController, curve: Curves.easeOut);
    offsetAnimation = Tween(begin: start, end: end).animate(curve)
      ..addListener(() {
        if (!onResetControllerValue) {
          offsetDistance = offsetAnimation.value;
  ...
  void closeDrawer() {
    var start = offsetDistance;
    var end = defaultOffsetDistance;
    debugPrint('closeDrawer start:${start},end:${end}');
    dragToggleController.reset();
    final CurvedAnimation curve = CurvedAnimation(parent: dragToggleController, curve: Curves.easeOut);
    offsetAnimation = Tween(begin: start, end: end).animate(curve)
      ..addListener(() {
        debugPrint('closeDrawer offsetDistance:${offsetDistance}');
        if (!onResetControllerValue) {
          offsetDistance = offsetAnimation.value;
          setState(() {});
        }
